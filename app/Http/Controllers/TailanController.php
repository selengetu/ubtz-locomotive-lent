    ...
    public function index()
{
    ...
    $achaa2=DB::select("select count (q2.route_id) as count from
       ...
       t.is_fault,
       t.train_no,
       SUBSTR(t.workid, 1, 1) from RIBBON t, ZUTGUUR.Marshbrig b where SUBSTR(t.workid, 1, 1) not in (1,5) and b.marshyear=2019 and b.marshmonth in (1,2,3,4,5,6) and b.marshid=t.route_id and b.depocode=".Auth::user()->depo_id. ") q2");
    ...
    $achaa3=DB::select("select count (q2.route_id) as count from
       ...
       t.is_fault,
       t.train_no,
       SUBSTR(t.workid, 1, 1) from RIBBON t, ZUTGUUR.Marshbrig b where SUBSTR(t.workid, 1, 1) not in (1,5) and b.marshyear=2019 and b.marshmonth in (4,5,6) and b.marshid=t.route_id and b.depocode=".Auth::user()->depo_id. ") q2");
    ...
    $suudal2=DB::select("select count (q2.route_id) as count from
       ...
       t.is_fault,
       t.train_no,
       SUBSTR(t.workid, 1, 1) from RIBBON t, ZUTGUUR.Marshbrig b where SUBSTR(t.workid, 1, 1) =1 and b.marshyear=2019 and b.marshmonth in (1,2,3,4,5,6) and b.marshid=t.route_id and b.depocode=".Auth::user()->depo_id. ") q2");
    $suudal3=DB::select("select count (q2.route_id) as count from
       ...
       t.is_fault,
       t.train_no,
       SUBSTR(t.workid, 1, 1) from RIBBON t, ZUTGUUR.Marshbrig b where SUBSTR(t.workid, 1, 1) =1 and b.marshyear=2019 and b.marshmonth in (4,5,6) and b.marshid=t.route_id and b.depocode=".Auth::user()->depo_id. ") q2");
    ...
    $selgee2 =DB::select("select count (q2.route_id) as count from
       ...
       t.is_fault,
       t.train_no,
       SUBSTR(t.workid, 1, 1) from RIBBON t, ZUTGUUR.Marshbrig b where SUBSTR(t.workid, 1, 1) =5 and b.marshyear=2019 and b.marshmonth in (1,2,3,4,5,6) and b.marshid=t.route_id and b.depocode=".Auth::user()->depo_id. ") q2");
    $selgee3 =DB::select("select count (q2.route_id) as count from
       ...
       t.is_fault,
       t.train_no,
       SUBSTR(t.workid, 1, 1) from RIBBON t, ZUTGUUR.Marshbrig b where SUBSTR(t.workid, 1, 1) =5 and b.marshyear=2019 and b.marshmonth in (4,5,6) and b.marshid=t.route_id and b.depocode=".Auth::user()->depo_id. ") q2");
    ...
    $niit2 = DB::select("select count (q2.route_id) as count from
       ...
       t.is_fault,
       t.train_no,
       SUBSTR(t.workid, 1, 1) from RIBBON t, ZUTGUUR.Marshbrig b where SUBSTR(t.workid, 1, 1) is not null and b.marshyear=2019 and b.marshmonth in (1,2,3,4,5,6) and b.marshid=t.route_id and b.depocode=".Auth::user()->depo_id. ") q2");
    $niit3 = DB::select("select count (q2.route_id) as count from
       ...
       t.is_fault,
       t.train_no,
       SUBSTR(t.workid, 1, 1) from RIBBON t, ZUTGUUR.Marshbrig b where SUBSTR(t.workid, 1, 1) is not null and b.marshyear=2019 and b.marshmonth in (4,5,6) and b.marshid=t.route_id and b.depocode=".Auth::user()->depo_id. ") q2");
    ...
    $hurd2 =DB::select("select b.broketype_id, b.broketype_name, count(o.broketype) as cnt from    
                            ...
                            where t.depo_id=g.depocode
                                  ...
                                  and g.marshmonth in (1,2,3,4,5,6)
                                  and g.depocode=".Auth::user()->depo_id. " 
                                  and f.fault_no=36
                                ) o,
                            broke_type b  
    ...
    $yaraltai362 =DB::select("select s.stop_id, s.stop_name ,count(q.broketype) as COUNT from
                                ...
                                inner join fault f on f.ribbon_id = t.ribbon_id
                                left join fault_det d on d.fault_id=f.fault_id
                                where  f.fault_no=35 and d.broketype=36 and g.depocode=t.depo_id and g.marshyear=2019 and g.marshmonth in (1,2,3,4,5,6) and g.depocode=".Auth::user()->depo_id. "
                                group by d.is_stop, g.depocode,d.broketype,t.route_id ) q
                                right join stop_det s
                                on s.stop_id=q.is_stop
                                group by s.stop_name, q.depocode,s.stop_id");
    $yaraltai363 =DB::select("select s.stop_id, s.stop_name ,count(q.broketype) as COUNT from
                                ...
                                inner join fault f on f.ribbon_id = t.ribbon_id
                                left join fault_det d on d.fault_id=f.fault_id
                                where  f.fault_no=35 and d.broketype=36 and g.depocode=t.depo_id and g.marshyear=2019 and g.marshmonth in (4,5,6) and g.depocode=".Auth::user()->depo_id. "
                                group by d.is_stop, g.depocode,d.broketype,t.route_id ) q
                                right join stop_det s
                                on s.stop_id=q.is_stop
                                group by s.stop_name, q.depocode,s.stop_id");
    ...
    $yaraltai352 =DB::select("select s.stop_id, s.stop_name ,count(q.broketype) as COUNT from
                                ...
                                inner join fault f on f.ribbon_id = t.ribbon_id
                                left join fault_det d on d.fault_id=f.fault_id
                                where  f.fault_no=35 and d.broketype=35 and g.depocode=t.depo_id  and g.marshyear=2019 and g.marshmonth in (1,2,3,4,5,6) and g.depocode=".Auth::user()->depo_id. "
                                group by d.is_stop, g.depocode,d.broketype,t.route_id ) q
                                right join stop_det s
                                on s.stop_id=q.is_stop
                                group by s.stop_name, q.depocode,s.stop_id");
    ...
    $yaraltai353 =DB::select("select s.stop_id, s.stop_name ,count(q.broketype) as COUNT from
                                ...
                                inner join fault f on f.ribbon_id = t.ribbon_id
                                left join fault_det d on d.fault_id=f.fault_id
                                where  f.fault_no=35 and d.broketype=35 and g.depocode=t.depo_id  and g.marshyear=2019 and g.marshmonth in (4,5,6) and g.depocode=".Auth::user()->depo_id. "
                                group by d.is_stop, g.depocode,d.broketype,t.route_id ) q
                                right join stop_det s
                                on s.stop_id=q.is_stop
                                group by s.stop_name, q.depocode,s.stop_id");
    ...
    $yaraltai372 =DB::select("select s.stop_id, s.stop_name ,count(q.broketype) as COUNT from
                                ...
                                inner join fault f on f.ribbon_id = t.ribbon_id
                                left join fault_det d on d.fault_id=f.fault_id
                                where  f.fault_no=35 and d.broketype=37 and g.depocode=t.depo_id and g.marshyear=2019 and g.marshmonth in (1,2,3,4,5,6) and g.depocode=".Auth::user()->depo_id. "
                                group by d.is_stop, g.depocode,d.broketype,t.route_id ) q
                                right join stop_det s
                                on s.stop_id=q.is_stop
                                group by s.stop_name, q.depocode,s.stop_id");
    ...
    $yaraltai373 =DB::select("select s.stop_id, s.stop_name ,count(q.broketype) as COUNT from
                                ...
                                inner join fault f on f.ribbon_id = t.ribbon_id
                                left join fault_det d on d.fault_id=f.fault_id
                                where  f.fault_no=35 and d.broketype=37 and g.depocode=t.depo_id and g.marshyear=2019 and g.marshmonth in (4,5,6) and g.depocode=".Auth::user()->depo_id. "
                                group by d.is_stop, g.depocode,d.broketype,t.route_id ) q
                                right join stop_det s
                                on s.stop_id=q.is_stop
                                group by s.stop_name, q.depocode,s.stop_id");
    ...
    $yaraltai382 =DB::select("select s.stop_id, s.stop_name ,count(q.broketype) as COUNT from
                                ...
                                inner join fault f on f.ribbon_id = t.ribbon_id
                                left join fault_det d on d.fault_id=f.fault_id
                                where  f.fault_no=35 and d.broketype=38 and g.depocode=t.depo_id and g.marshyear=2019 and g.marshmonth in (1,2,3,4,5,6) and g.depocode=".Auth::user()->depo_id. "
                                group by d.is_stop, g.depocode,d.broketype,t.route_id ) q
                                right join stop_det s
                                on s.stop_id=q.is_stop
                                group by s.stop_name, q.depocode,s.stop_id");
    ...
    $yaraltai383 =DB::select("select s.stop_id, s.stop_name ,count(q.broketype) as COUNT from
                                ...
                                inner join fault f on f.ribbon_id = t.ribbon_id
                                left join fault_det d on d.fault_id=f.fault_id
                                where  f.fault_no=35 and d.broketype=38 and g.depocode=t.depo_id and g.marshyear=2019 and g.marshmonth in (4,5,6) and g.depocode=".Auth::user()->depo_id. "
                                group by d.is_stop, g.depocode,d.broketype,t.route_id ) q
                                right join stop_det s
                                on s.stop_id=q.is_stop
                                group by s.stop_name, q.depocode,s.stop_id");
    ...
    $zurchil2 =DB::select("select
                                        ...
                                        ribbon t,  
                                         ...
                                         where t.ribbon_id=f.ribbon_id
                                          and b.marshid=t.route_id  
                                          and  b.marshyear=2019 and b.marshmonth in (1,2,3,4,5,6) and b.depocode=".Auth::user()->depo_id. "
                                          ) q 
                                          left join fault_det de on de.fault_id=q.fault_id
                                          where de.is_techact is null or de.is_techact=2) q1 right join
                                         fault_detail d on d.fault_detail_id=q1.fault_no
    ...
    $zurchil3  =DB::select("select
                                        ...
                                        ribbon t,  
                                         ...
                                         where t.ribbon_id=f.ribbon_id
                                          and b.marshid=t.route_id  
                                          and  b.marshyear=2019 and b.marshmonth in (4,5,6) and b.depocode=".Auth::user()->depo_id. "
                                          ) q 
                                          left join fault_det de on de.fault_id=q.fault_id
                                          where de.is_techact is null or de.is_techact=2) q1 right join
                                         fault_detail d on d.fault_detail_id=q1.fault_no
  ...
  (select
   ...
   from FAULT f, ribbon t, fault_detail e, ZUTGUUR.Marshbrig b
   where t.ribbon_id=f.ribbon_id and e.fault_detail_id=f.fault_no 
   and b.marshid=t.route_id and f.fault_no in (32,17,33,37,26,25,31,22,16,13,23,14,20,18,19,15,30,34,41,161) and b.marshyear=2019 and b.marshmonth in (1,2,3,4,5,6) and b.depocode=".Auth::user()->depo_id. ") q
   left join fault_det d on d.fault_id=q.fault_id
   where d.is_techact = 2 or d.is_techact is null ");
   ...
   from FAULT f, ribbon t, fault_detail e, ZUTGUUR.Marshbrig b
   where t.ribbon_id=f.ribbon_id and e.fault_detail_id=f.fault_no 
   and b.marshid=t.route_id and f.fault_no in (32,17,33,37,26,25,31,22,16,13,23,14,20,18,19,15,30,34,41,161) and b.marshyear=2019 and b.marshmonth in (4,5,6) and b.depocode=".Auth::user()->depo_id. ") q
   left join fault_det d on d.fault_id=q.fault_id
   where d.is_techact = 2 or d.is_techact is null ");
    ...
    $orohachaa2 =DB::select("select count(q2.route_id) as too from (select
                                        ...
                                        inner join ZUTGUUR.MARSHBRIG g on g.marshid=t.route_id
                                        inner join fault f on f.ribbon_id = t.ribbon_id
                                        where  g.marshyear=2019 and g.marshmonth in (1,2,3,4,5,6) and g.depocode=".Auth::user()->depo_id. " and f.fault_no=21 and SUBSTR(t.workid, 1, 1)!= 1
                                        group by t.route_id, t.workid) q2");
    $orohachaa3 =DB::select("select count(q2.route_id) as too from (select
                                        ...
                                        inner join ZUTGUUR.MARSHBRIG g on g.marshid=t.route_id
                                        inner join fault f on f.ribbon_id = t.ribbon_id
                                        where  g.marshyear=2019 and g.marshmonth in (4,5,6) and g.depocode=".Auth::user()->depo_id. " and f.fault_no=21 and SUBSTR(t.workid, 1, 1)!= 1
                                        group by t.route_id, t.workid) q2");
    ...
    $orohniit2 =DB::select("select count(q2.route_id) as too from (select
                                        ...
                                        inner join ZUTGUUR.MARSHBRIG g on g.marshid=t.route_id
                                        inner join fault f on f.ribbon_id = t.ribbon_id
                                        where g.marshyear=2019 and g.marshmonth in (1,2,3,4,5,6) and g.depocode=".Auth::user()->depo_id. " and f.fault_no=21 and t.workid is not null
                                        group by t.route_id, t.workid) q2");
    $orohniit3 =DB::select("select count(q2.route_id) as too from (select
                                        ...
                                        inner join ZUTGUUR.MARSHBRIG g on g.marshid=t.route_id
                                        inner join fault f on f.ribbon_id = t.ribbon_id
                                        where g.marshyear=2019 and g.marshmonth in (4,5,6) and g.depocode=".Auth::user()->depo_id. " and f.fault_no=21 and t.workid is not null
                                        group by t.route_id, t.workid) q2");
    ...
    $uharsan2 =DB::select("select
                                    ...
                                    LEFT JOIN V_broketype b on b.broketype_id= d.broketype
                                    where t.depo_id=g.depocode and e.depocode=t.depo_id and e.marshyear=g.marshyear and e.marshmonth=g.marshmonth and f.fault_no=8  and e.marshyear=2019 and e.marshmonth in (1,2,3,4,5,6) and e.depocode=".Auth::user()->depo_id. "
                                    ");
...
group by q2.depo_id,q2.marshyear, q2.marshmonth");
    $hoorond2 =DB::select("select
                                ...
                                inner join fault f on f.ribbon_id = t.ribbon_id
                                left join fault_det d on d.fault_id=f.fault_id 
                                where t.depo_id=g.depocode and e.depocode=t.depo_id and e.marshyear=g.marshyear and e.marshmonth=g.marshmonth and f.fault_no=2 and e.marshyear=2019 and e.marshmonth in (1,2,3,4,5,6) and e.depocode=".Auth::user()->depo_id. "
                                    ");
    ...
    $techno2 =DB::select("select
                                ...
                                inner join fault f on f.ribbon_id = t.ribbon_id
                                left join fault_det d on d.fault_id=f.fault_id
                                where t.depo_id=g.depocode and e.depocode=t.depo_id and e.marshyear=g.marshyear and e.marshmonth=g.marshmonth and f.fault_no=12 and e.marshyear=2019 and e.marshmonth in (1,2,3,4,5,6) and e.depocode=".Auth::user()->depo_id. "
                                    ");
    ...
    $tuslamjzammin2 =DB::select("select
                             sum(substr(k.stoptime,4,2)+((substr(k.stoptime,1,2))*60)) as too
                                from FAULT f, ribbon t, fault_detail e, V_Marshbrig b, fault_det k
                                where t.ribbon_id=f.ribbon_id and e.fault_detail_id=f.fault_no and k.fault_id=f.fault_id and b.marshid=t.route_id and f.fault_no=3 and b.marshyear=2019 and b.marshmonth in (1,2,3,4,5,6) and b.depocode=".Auth::user()->depo_id. " ");
    $tuslamjzam2 =DB::select("select
                                f.fault_no,
                                e.fault_detail_name,
                                count(f.fault_no) as too
                                from FAULT f, ribbon t, fault_detail e, V_Marshbrig b
                                where t.ribbon_id=f.ribbon_id and e.fault_detail_id=f.fault_no and b.marshid=t.route_id and f.fault_no=3 and  b.marshyear=2019 and b.marshmonth in (1,2,3,4,5,6) and b.depocode=".Auth::user()->depo_id. "
                                group by f.fault_no, e.fault_detail_name");
    $tuslamjurtuu2 =DB::select("select
                                ...
                                count(f.fault_no) as too
                                from FAULT f, ribbon t, fault_detail e, V_Marshbrig b
                                where t.ribbon_id=f.ribbon_id and e.fault_detail_id=f.fault_no and b.marshid=t.route_id and f.fault_no=4 and  b.marshyear=2019 and b.marshmonth in (1,2,3,4,5,6) and b.depocode=".Auth::user()->depo_id. "
                                group by f.fault_no, e.fault_detail_name");
    $tuslamjurtuumin2 =DB::select("select
                              sum(substr(k.stoptime,4,2)+((substr(k.stoptime,1,2))*60)) as too
                                from FAULT f, ribbon t, fault_detail e, V_Marshbrig b, fault_det k
                                where t.ribbon_id=f.ribbon_id and e.fault_detail_id=f.fault_no and k.fault_id=f.fault_id and b.marshid=t.route_id and f.fault_no=4 and b.marshyear=2019 and b.marshmonth in (1,2,3,4,5,6) and b.depocode=".Auth::user()->depo_id. " ");
    $speed2 =DB::select("select p.attentionspeed_id, nvl(a.speed,0) as niit, count(a.speed) as cnt
                            from (select a.speed, a.fromstation, a.tostation
                            from Attention a, ribbon t, ZUTGUUR.Marshbrig g
                            where a.ribbon_id=t.ribbon_id 
                            and g.marshid=t.route_id
                            and  g.marshyear=2019 and g.marshmonth in (1,2,3,4,5,6) and g.depocode=".Auth::user()->depo_id. ") a,
                            
                            attention_speed p
                            
                            where p.attentionspeed_id=a.speed(+)
                            group by p.attentionspeed_id, a.speed");
    $hurdniit2 =DB::select("select count(route_id) as too from zurchil_hurdhemjigch t where   t.marshyear=2019 and t.marshmonth in (1,2,3,4,5,6) and t.depocode=".Auth::user()->depo_id. "");
    $yaraltainiit2 =DB::select("select count(route_id) as too from  ribbon t
                                ...
                                left join fault_det d on d.fault_id=f.fault_id
                                LEFT JOIN V_Broketype b on b.broketype_id= d.broketype
                                where t.depo_id=g.depocode and e.depocode=t.depo_id and e.marshyear=g.marshyear and e.marshmonth=g.marshmonth and   e.marshyear=2019 and e.marshmonth in (1,2,3,4,5,6) and t.depo_id=".Auth::user()->depo_id. " and f.fault_no=35");
...
(select distinct t.route_id, t.depo_id, g.marshyear, g.marshmonth ,t.locserial, t.zutnumber, t.patchmin from RIBBON t , ZUTGUUR.MARSHBRIG g
where t.route_id = g.marshid and t.patchmin is not null and t.patchmin != '0' and t.patchmin != '00:00:00' and  g.marshyear=2019 and g.marshmonth in (1,2,3,4,5,6) and g.depocode=".Auth::user()->depo_id. ") q2     
group by q2.depo_id,q2.marshyear, q2.marshmonth");
    $hoorond3 =DB::select("select
                                ...
                                inner join fault f on f.ribbon_id = t.ribbon_id
                                left join fault_det d on d.fault_id=f.fault_id 
                                where t.depo_id=g.depocode and e.depocode=t.depo_id and e.marshyear=g.marshyear and e.marshmonth=g.marshmonth and f.fault_no=2 and e.marshyear=2019 and e.marshmonth in (4,5,6) and e.depocode=".Auth::user()->depo_id. "
                                    ");
    ...
    $techno3 =DB::select("select
                                ...
                                inner join fault f on f.ribbon_id = t.ribbon_id
                                left join fault_det d on d.fault_id=f.fault_id
                                where t.depo_id=g.depocode and e.depocode=t.depo_id and e.marshyear=g.marshyear and e.marshmonth=g.marshmonth and f.fault_no=12 and e.marshyear=2019 and e.marshmonth in (4,5,6) and e.depocode=".Auth::user()->depo_id. "
                                    ");
    ...
    $tuslamjzammin3 =DB::select("select
                             sum(substr(k.stoptime,4,2)+((substr(k.stoptime,1,2))*60)) as too
                                from FAULT f, ribbon t, fault_detail e, V_Marshbrig b, fault_det k
                                where t.ribbon_id=f.ribbon_id and e.fault_detail_id=f.fault_no and k.fault_id=f.fault_id and b.marshid=t.route_id and f.fault_no=3 and b.marshyear=2019 and b.marshmonth in (4,5,6) and b.depocode=".Auth::user()->depo_id. " ");
    $tuslamjzam3 =DB::select("select
                                f.fault_no,
                                e.fault_detail_name,
                                count(f.fault_no) as too
                                from FAULT f, ribbon t, fault_detail e, V_Marshbrig b
                                where t.ribbon_id=f.ribbon_id and e.fault_detail_id=f.fault_no and b.marshid=t.route_id and f.fault_no=3 and  b.marshyear=2019 and b.marshmonth in (4,5,6) and b.depocode=".Auth::user()->depo_id. "
                                group by f.fault_no, e.fault_detail_name");
    $tuslamjurtuu3 =DB::select("select
                                ...
                                count(f.fault_no) as too
                                from FAULT f, ribbon t, fault_detail e, V_Marshbrig b
                                where t.ribbon_id=f.ribbon_id and e.fault_detail_id=f.fault_no and b.marshid=t.route_id and f.fault_no=4 and  b.marshyear=2019 and b.marshmonth in (4,5,6) and b.depocode=".Auth::user()->depo_id. "
                                group by f.fault_no, e.fault_detail_name");
    $tuslamjurtuumin3 =DB::select("select
                              sum(substr(k.stoptime,4,2)+((substr(k.stoptime,1,2))*60)) as too
                                from FAULT f, ribbon t, fault_detail e, V_Marshbrig b, fault_det k
                                where t.ribbon_id=f.ribbon_id and e.fault_detail_id=f.fault_no and k.fault_id=f.fault_id and b.marshid=t.route_id and f.fault_no=4 and b.marshyear=2019 and b.marshmonth in (4,5,6) and b.depocode=".Auth::user()->depo_id. " ");
    $speed3 =DB::select("select p.attentionspeed_id, nvl(a.speed,0) as niit, count(a.speed) as cnt
                            from (select a.speed, a.fromstation, a.tostation
                            from Attention a, ribbon t, ZUTGUUR.Marshbrig g
                            where a.ribbon_id=t.ribbon_id 
                            and g.marshid=t.route_id
                            and  g.marshyear=2019 and g.marshmonth in (4,5,6) and g.depocode=".Auth::user()->depo_id. ") a,
                            
                            attention_speed p
                            
                            where p.attentionspeed_id=a.speed(+)
                            group by p.attentionspeed_id, a.speed");
    $hurdniit3 =DB::select("select count(route_id) as too from zurchil_hurdhemjigch t where   t.marshyear=2019 and t.marshmonth in (4,5,6) and t.depocode=".Auth::user()->depo_id. "");
    $yaraltainiit3 =DB::select("select count(route_id) as too from  ribbon t
                                ...
                                left join fault_det d on d.fault_id=f.fault_id
                                LEFT JOIN V_Broketype b on b.broketype_id= d.broketype
                                where t.depo_id=g.depocode and e.depocode=t.depo_id and e.marshyear=g.marshyear and e.marshmonth=g.marshmonth and   e.marshyear=2019 and e.marshmonth in (4,5,6) and t.depo_id=".Auth::user()->depo_id. " and f.fault_no=35");
...
(select distinct t.route_id, t.depo_id, g.marshyear, g.marshmonth ,t.locserial, t.zutnumber, t.patchmin from RIBBON t , ZUTGUUR.MARSHBRIG g
where t.route_id = g.marshid and t.patchmin is not null and t.patchmin != '0' and t.patchmin != '00:00:00' and  g.marshyear=2019 and g.marshmonth in (4,5,6) and g.depocode=".Auth::user()->depo_id. ") q2     
group by q2.depo_id,q2.marshyear, q2.marshmonth");
     ...
     public function orohdohio()
{
    ...
    public function norm()
    {
        ...
        if ($startdate !=0 && $startdate && $enddate !=0 && $enddate !=NULL) {
            $query.=  "and c.depdatetime between '".$startdate." 00:00:00' and '".$enddate." 23:59:59'";
        }
        else
        {
            $query.=" and c.depdatetime between sysdate-10 and sysdate";
            $startdate= Carbon::today()->subDays(10)->toDateString();
            $enddate=  Carbon::today()->toDateString();
